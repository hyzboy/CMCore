...
namespace hgl
{
    ...
    ///        EN: Non-owning string view based on std::basic_string_view that provides efficient read-only string utilities。
    ///
    /// CN: 注意 StringView 不拥有底层数据，使用时必须保证被引用的数据在视图生命周期内有效。
    /// EN: Note that StringView does not own underlying data; ensure the referenced data outlives the view.
    ///
    ...
    public:
        ...
        /// @return CN: 无返回值，构造出视图对象；若 s 为 nullptr 或长度为 0 则为空视图。
        ///         EN: No return; constructs the view. Empty view is created when s is nullptr or length is 0.
        ...
        ///        EN: Write substring into output parameter to avoid return construction overhead.
        ...
        ///        EN: Find character ch starting at pos and return offset relative to pos; -1 if not found.
        ...
        /// @brief CN: 查找子视图 ch 在本视图中的首次出现（从 pos 开始），返回相对于 pos 的偏移或 -1。
        ///        EN: Find first occurrence of substring view ch starting at pos; return offset relative to pos or -1.
        /// @param pos CN: 起始偏移。
        ///            EN: Start offset.
        /// @param ch CN: 要查找的子视图。
        ...
        ///        EN: Find first occurrence of view str in this view; return absolute offset or -1 if not found.
        ...
        // EN: String to bool/int/float conversion
        ...
        ///        EN: Return a new view with the first n characters removed (does not modify original view).
        /// @param n CN: 要剔除的前缀长度。
        ///          EN: Number of characters to remove from prefix.
        /// @return CN: 剔除后的新视图；若 n<=0 返回原视图，n>=Length() 返回空视图。
        ///         EN: New view after removal; if n<=0 returns original view, if n>=Length() returns empty view.
        ...
        ///        EN: Return a new view with the last n characters removed (does not modify original view).
        /// @param n CN: 要剔除的后缀长度。
        ...
        /// @note CN: 若 n >= Length() 则视图变为空；若 n<=0 则无操作。
        ///       EN: If n >= Length() view becomes empty; if n<=0 no-op.

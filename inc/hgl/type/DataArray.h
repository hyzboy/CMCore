...
namespace hgl
{
    ...
    public:
        ...
        int64 Find(const T &data,const int64 start=0,int64 find_count=-1) const
        {
            ...
            if(find_count<0||find_count>count-start) find_count=count-start;
        ...
        bool Reserve(int64 size)
        {
            ...
            if(size<=alloc_count) return true; // never shrink
        ...
        int64 Resize(int64 size)
        {
            ...
            if(size>alloc_count) Reserve(size);
            if(size>old_count)
        ...
        void Free()
        {
            if(items){ hgl_free(items); items=nullptr; alloc_count=0; }
            count=0;
        }
        void Clear(){ count=0; }
        ...
        bool Insert(int64 pos,const T *data,const int64 data_number)
        {
            ...
            if(count+data_number>alloc_count) Reserve(count+data_number);
            if(pos<count)
            {
        ...
        void Move(int64 new_index,int64 old_index,int64 move_number=1)
        {
            ...
            if(new_index<0) new_index=0; if(new_index>=count) new_index=count;
            if(old_index+move_number>count) move_number=count-old_index;
        ...
        void WithoutList(DataArray<T> &result_list,const DataArray<T> &without_list)
        {
            ...
            result_list.Clear(); result_list.Reserve(local_count);
            for(T &sp:*this)
            {

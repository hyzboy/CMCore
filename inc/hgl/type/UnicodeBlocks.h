...
namespace hgl
{
    ...
    struct UnicodeBlock
    {
        UnicodeBlockType type;
        uint32 begin;
        uint32 end;
        char name[64];
    };//struct UnicodeBlock

    #define UNICODE_BLOCK_DEFINE(begin,end,intro)   {UnicodeBlockType::intro,0x##begin,0x##end,#intro}

    constexpr UnicodeBlock UnicodeBlockList[(size_t)UnicodeBlockType::RANGE_SIZE]=
    {
    ...
    inline bool IsInUnicodeBlock(const UnicodeBlockType &type,const uint32 ch)
    {
        if(type<UnicodeBlockType::basic_latin
         ||type>UnicodeBlockType::END_RANGE)
            return(false);
    ...
    inline bool isCJKChar(const u16char ch)
    {
        if(IsInUnicodeBlock(UnicodeBlockType::cjk_radicals_supplement))return(true);
        if(IsInUnicodeBlock(UnicodeBlockType::cjk_symbols_and_punctuation))return(true);
        if(IsInUnicodeBlock(UnicodeBlockType::cjk_strokes))return(true);
        if(IsInUnicodeBlock(UnicodeBlockType::enclosed_cjk_letters_and_months))return(true);
        if(IsInUnicodeBlock(UnicodeBlockType::cjk_compatibility))return(true);
        if(IsInUnicodeBlock(UnicodeBlockType::cjk_unified_ideographs_extension_a))return(true);
        if(IsInUnicodeBlock(UnicodeBlockType::cjk_unified_ideographs))return(true);
        if(IsInUnicodeBlock(UnicodeBlockType::cjk_compatibility_ideographs))return(true);
        if(IsInUnicodeBlock(UnicodeBlockType::cjk_compatibility_forms))return(true);

        return(false);
    }

    inline bool isCJKChar(const u32char ch)
    {
        if(ch<0x10000)
            return isCJKChar((const u16char)ch);

        if(IsInUnicodeBlock(UnicodeBlockType::cjk_unified_ideographs_extension_b))return(true);
        if(IsInUnicodeBlock(UnicodeBlockType::cjk_unified_ideographs_extension_c))return(true);
        if(IsInUnicodeBlock(UnicodeBlockType::cjk_unified_ideographs_extension_d))return(true);
        if(IsInUnicodeBlock(UnicodeBlockType::cjk_unified_ideographs_extension_e))return(true);
        if(IsInUnicodeBlock(UnicodeBlockType::cjk_unified_ideographs_extension_f))return(true);
        if(IsInUnicodeBlock(UnicodeBlockType::cjk_compatibility_ideographs_supplement))return(true);
        if(IsInUnicodeBlock(UnicodeBlockType::cjk_unified_ideographs_extension_g))return(true);

/**
 * @file StaticPerfectHashMapBuilder.h
 * @brief 静态完美哈希映射构建器 - 编辑器工具（最小原型）
 * 
 * 实现简化的CHD算法，用于构建完美哈希表。
 */
#pragma once
...
namespace hgl
{
    ...
    class StaticPerfectHashMapBuilder
    {
    private:
        std::vector<std::pair<K, V>> data;  // 输入数据
        std::vector<uint16_t> displacement_table;  // 位移表
        std::vector<K> sorted_keys;         // 排列后的键
        std::vector<V> sorted_values;       // 对应的值
        uint32_t num_buckets;               // 桶数量
        BuildStats build_stats;             // 构建统计
    ...
    public:
        StaticPerfectHashMapBuilder() = default;
                ...
                for (uint32_t disp = 0; disp < max_tries; ++disp)
                {
                    ...
                    if (!collision)
                    {
                        ...
                        for (size_t i = 0; i < bucket.size(); ++i)
                        {
                            ...
                            occupied[pos] = true;
                            sorted_keys[pos] = bucket[i].first;
                            sorted_values[pos] = bucket[i].second;
                        }
    ...
    public:
        ...
        size_t SaveToMemory(std::vector<uint8_t>& buffer) const
        {
            ...
            typename StaticPerfectHashMap<K, V>::FileHeader header;
            std::memcpy(header.magic, "SPHM", 4);
            header.version = 1;
            ...
            // 写入值数组
            std::memcpy(ptr, sorted_values.data(), 
                       header.num_keys * sizeof(V));
        ...
        Stats GetStats() const
        {
            ...
            stats.total_size = sizeof(typename StaticPerfectHashMap<K, V>::FileHeader) +
                              num_buckets * sizeof(uint16_t) +
                              stats.num_keys * sizeof(K) +
                              stats.num_keys * sizeof(V);
            
            size_t raw_data_size = stats.num_keys * (sizeof(K) + sizeof(V));
